...
The Dashboard's review metric is a live `Ôn lại sau` countdown to the earliest scheduled non-new FSRS card in the active Study Scope. It shows `Đã đến lúc ôn` when any included review is due, and `Chưa có lịch ôn` when no valid scheduled review exists. The countdown refreshes every minute and when the tab becomes visible; persisted timestamps remain the scheduling source of truth.
