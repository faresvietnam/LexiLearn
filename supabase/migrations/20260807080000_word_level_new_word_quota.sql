-- daily_new_word_usage.completed_count was incremented once per learning
-- card (one per meaning) that left the "New" fsrs_state, so a word with
-- multiple meanings spent multiple "new word" quota slots for what the
-- UI presents as a single word. Only charge the quota on a card's
-- completion when none of its sibling cards (same personal_vocabulary_id,
-- i.e. the same word) have already left the "New" state — the word's
-- first completed meaning pays the slot, later meanings of that same
-- word are free.
...
declare
  ...
  v_personal_vocabulary_id uuid;
  v_word_already_counted boolean;
begin
  ...
  select fsrs_state, personal_vocabulary_id into v_previous_state, v_personal_vocabulary_id
  from public.learning_cards
  ...
  if p_is_new_word and v_previous_state = 0 then
    select exists (
      select 1
      from public.learning_cards sibling
      where sibling.personal_vocabulary_id = v_personal_vocabulary_id
        and sibling.user_id = v_user_id
        and sibling.id <> p_learning_card_id
        and sibling.fsrs_state <> 0
    ) into v_word_already_counted;

    if not v_word_already_counted then
