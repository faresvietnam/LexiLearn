-- create_private_word cast each tag_ids element via jsonb_array_elements
-- (returns jsonb) then value::text, which keeps the JSON string's quote
-- characters (e.g. `"4cc368c4-..."` instead of `4cc368c4-...`), so the
-- ::uuid cast always failed with "invalid input syntax for type uuid" for
-- any word carrying at least one tag. jsonb_array_elements_text returns the
-- unquoted text directly, matching the pattern already used elsewhere
-- (see add_submit_learning_review_rpc.sql).
