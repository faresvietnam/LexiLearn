...
as $$
  select
    ...
    coalesce(vocabulary.total_count, 0)::bigint as vocabulary_count,
    coalesce(memory.remembered_count, 0)::bigint as remembered_word_count,
    coalesce(new_words.average_count, 0)::numeric as average_new_words_per_study_day
  from public.users u
    ...
    from (
      select
        ((a.created_at at time zone coalesce(nullif(u.timezone, ''), 'Asia/Ho_Chi_Minh'))::date
          - case when (a.created_at at time zone coalesce(nullif(u.timezone, ''), 'Asia/Ho_Chi_Minh'))::time < u.study_day_starts_at then 1 else 0 end) as study_date,
